...
method_list = ["multiplicative", "additive"]
...
outdir_base = "/home/crwentl/research/code/pressio-proj/pdas-experiments/cases/mmldecset23/swe/figures/plots/error_method"
...
error_decomp_spacetime = np.zeros((nvars, 2, nparams), dtype=np.float64)
...
for param_idx, param in enumerate(paramlist):
    ...
    for method_idx, method in enumerate(method_list):
        ...
        datadir_decomp = os.path.join(datadir_base, "decomp", meshname, f"{ndomX}x{ndomY}", f"overlap{overlap}", casename.format(param=param), method)
        ...
        if param_idx == 0:
            nsamps = errorlist[0].shape[0]
            error_decomp_space = np.zeros((nsamps, nvars, 2, nparams), dtype=np.float64)
        error_decomp_space[:, :, method_idx, param_idx] = errorlist[0]
        ...
        error_decomp_spacetime[:, method_idx, param_idx] = errorlist[0]

legend_labels = ["Monolithic", "Multiplicative", "Additive"]
...
for var_idx in range(nvars):
    ...
    for method_idx, method in enumerate(method_list):
        ax.semilogy(t, error_decomp_space[:, var_idx, method_idx, timeplot_param_idx], colorlist[method_idx], linestyle=linestylelist[method_idx])
    ...
    for method_idx, method in enumerate(method_list):
        ax.semilogy(paramlist, error_decomp_spacetime[var_idx, method_idx, :], colorlist[method_idx], linestyle=linestylelist[method_idx])
    ax.legend(legend_labels, fontsize=12)
